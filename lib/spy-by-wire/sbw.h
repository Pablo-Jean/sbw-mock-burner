...
typedef enum{
    ...
    IR_ADDR_16BIT = 0xC1,
    IR_ADDR_CAPTURE = 0x21,

    // Controlling the Memory Data Bus (MDB)
    IR_DATA_TO_ADDR = 0xA1,
    IR_DATA_16BIT = 0x82,
    ...
    IR_CNTRL_SIG_CAPTURE = 0x28,
    IR_CNTRL_SIG_RELEASE = 0xA8,

    // Memory Verification by Pseudo Signature Analysis (PSA)
    IR_DATA_PSA = 0x22,
    IR_SHIFT_OUT_PSA = 0x62,

    // JTAG Access Security Fuse Programming
    IR_Prepare_Blow = 0x44,
    IR_Ex_Blow = 0x24,

    // JTAG Mailbox System
    IR_JMB_EXCHANGE = 0x86,
    IR_TEST_REG = 0x54,
}jtag_mai_e;
